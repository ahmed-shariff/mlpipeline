...
'''
  ...
  def pre_execution_hook(self, version, model_dir, exec_mode=ExecutionModeKeys.TEST):
    '''
Before execution, this method will be called to set the version obtained from `self.versions`. Also `model_dir` will provide the destination to save the model in as specified in the config file. The exec_mode will be passed, with on of the keys as specified in `ExecutionModeKeys`. This function can be used to define the model based on the information of the version being executed duering an iteration. This method will be once called before `train_model` for each version. 
'''
    raise NotImplementedError
  
  def train_model(self, input_fn, steps):
    '''
This will be called when the model is entering the traning phase. Ideally, what needs to happen in this function is to use the input_fn to obtain the inputs and train the model for a given number of steps. The input function passed here will be the input function returned by the methods in the dataloader. In addition to that other functionalities can be included here as well, such as saving the model parameters during training, etc. Th return value of the method will be logged.
'''
    raise NotImplementedError
