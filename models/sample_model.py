...
import math
import itertools
from utils import model_fn_closing
from utils import Version
from utils import VersionContainer
MODEL_SUMMERY = ""
...
# VERSIONS.rangeOnParameter(["learning_rate-{0}".format(i) for i in range(3)],
#                           learning_rate = [0.01,0.001,0.0001])

#VERSIONS.rangeOnParameter(['class_offset_Grid_{0}'.format(i) for i in range(5)])
# level1_conv_layers=[[([128,128,100],[10,10])], []]
# level2_conv_layers=[[], [([32,32,100],[4,4])]]
# level3_dense_layers=[[], [1000,500]]

# #list(itertools.product(level1_conv_layers, level2_conv_layers, level3_dense_layers))
# products = [([([128, 128, 100], [10, 10])], [], []), ([([128, 128, 100], [10, 10])], [], [1000, 500]), ([([128, 128, 100], [10, 10])], [([32, 32, 100], [4, 4])], []), ([([128, 128, 100], [10, 10])], [([32, 32, 100], [4, 4])], [1000, 500])]
# for i in range(len(products)):
#   VERSIONS.addV("leyers_{0}".format(i))
#   VERSIONS.getVersion("leyers_{0}".format(i)).layers = products[i]

# for l_spec in [[2,4], [3,3], [2,3]]:
#   VERSIONS.addV("blk_size/kernal-{0}".format(l_spec))
#   VERSIONS.getVersion("blk_size/kernal-{0}".format(l_spec)).block_size=l_spec[0]
#   VERSIONS.getVersion("blk_size/kernal-{0}".format(l_spec)).kernal_size=l_spec[1]
